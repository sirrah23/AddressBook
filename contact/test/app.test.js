...
test('fail to update an non-existent contact', async done => {
    ...
    const res = await request(app)
                        .put('/api/v1/contact')
                        .set('Accept', 'application/json')
                        .send({userUUID: u1.uuid, contact: {id: insertedContactRes.body.contact.id+1, relationship: 'father'}})
    expect(res.status).toBe(400)
    expect(res.body.errorFlag).toBe(1)
...
test('successfully delete an existing contact', async done => {
    ...
    const res = await request(app)
                        ...
                        .set('Accept', 'application/json')
                        .send({userUUID: u1.uuid, contact: {id: insertedContactRes.body.contact.id}})
    expect(res.status).toBe(200)
    expect(res.body.errorFlag).toBe(0)
    const c1 = Contact.fetchById(insertedContactRes.body.contact.id)
    expect(c1).toBe(null)
    done()
})
...
test('fail to delete a non-existent contact', async done => {
    ...
    const res = await request(app)
                        .delete('/api/v1/contact')
                        .set('Accept', 'application/json')
                        .send({userUUID: u1.uuid, contact: {id: insertedContactRes.body.contact.id+1}})
    expect(res.status).toBe(400)
    expect(res.body.errorFlag).toBe(1)
    expect(res.body.message).toBe(`Contact with id ${insertedContactRes.body.contact.id} does not belong to user ${u2.uuid}`)
    done() 
